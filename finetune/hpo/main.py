...
def objective(trial):
    ...
    _r = trial.suggest_categorical("r", [8, 16, 32])
    _lora_alpha = trial.suggest_categorical("lora_alpha", [128])
    _lora_dropout = trial.suggest_categorical("lora_dropout", [0.05, 0.1])
    _target_modules = trial.suggest_categorical("target_modules", [
        "q_proj,k_proj,v_proj,o_proj,down_proj,up_proj,gate_proj"])

    _num_train_epochs = trial.suggest_categorical("num_train_epochs", [3, 5, 7])
    _gradient_accumulation_steps = trial.suggest_categorical("gradient_accumulation_steps", [1, 2, 4])
    ...
    command_str = (
        f"accelerate launch --config_file {trial.user_attrs['accelerate_config']} ./finetune/hpo/train.py "
